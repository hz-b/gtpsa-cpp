...
namespace gtpsa {
  ...
  public:
    ...
    inline void fgrad
    (typename T::tpsa_type * b, typename T::tpsa_type * r) const {
      typename T::bridge_container_type mc_c(this->getBridgePtrs());
      mc_c.fgrad(&b->m_impl, &r->m_impl);
    }

    inline void rliebra
    (const TpsaBridgeContainer<T> &ma, const TpsaBridgeContainer<T> &mb) {
    ...
    inline void rpminv
    (const TpsaBridgeContainer<T> &ma, std::vector<idx_t>& select) {
      const ContainerBridge ma_c(ma.getBridgePtrs());
      ContainerBridge mc_c(this->getBridgePtrs());
  ...
  protected:
    inline std::vector<typename T::bridge_base_type *>
    getBridgePtrs(void) const {
    ...
    inline std::vector<const typename T::bridge_base_type *>
    getBridgePtrsConst(void) const {
      std::vector<typename T::bridge_base_type *> vec(this->m_vec.size());
      std::transform(this->m_vec.begin(), this->m_vec.end(), vec.begin(),
