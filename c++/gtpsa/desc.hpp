...
namespace gtpsa::mad {
  ...
  public:
    ...
    inline ~desc_mgr(void) {
      if(!this->ptr){
#ifdef gtpsa_DEBUG
	std::cerr << "gtpsa::desc: would delete underlaying desc_t pointer a"
	  " second time" << std::endl;
#endif
	return;
      }
#ifdef gtpsa_DEBUG
#warning "gtpsa::desc: not deleting underlying gtpsa desc_t object. "	\
  ...
  public:
    inline desc_info
    (int nv, ord_t no, int np, ord_t po, const std::vector<ord_t> orders)
      : m_nv(nv)
      , m_np(np)
      , m_no(no)
      , m_po(po)
  ...
  public:
	...
	strm << "nv= "<< nv <<"+ np="<< np << "=" << req_elem
	     << "req. elem, got only "<<no.size()<<"elem";
	std::runtime_error(strm.str());
      }
      this->dm = std::make_unique<desc_mgr>
	(mad_desc_newvpo(nv, mo, np, po, no.data()));
    }
  ...
  public:
    ...
    inline int   getNv(ord_t *mo_=0, int *np_=0, ord_t *po_=0)
      const { return mad_desc_getnv (this->getPtr(), mo_, np_, po_); }
    // inline ord_t getNo(int nn, ord_t *no_) const
    // { return mad_desc_getno (this->getPtr(), nn, no_); }
    inline ord_t maxOrd(int nn=0, ord_t *no=nullptr) const
    { return mad_desc_maxord(this->getPtr(), nn, nullptr); }
    inline ssz_t maxLen(ord_t mo) const
    { return mad_desc_maxlen(this->getPtr(), mo); }
    // inline ssz_t ordLen(const ord_t mo) const
    // { return mad_desc_ordlen(this->getPtr(), mo); }
    inline ssz_t trunc(const ord_t to) const
    { return mad_desc_gtrunc(this->getPtr(), to); }
    ...
    inline void info(FILE * fp = nullptr) const
    { mad_desc_info(this->getPtr(), fp); }
    ...
    inline log_t isvalid   (const std::string& s, const ssz_t n=0) const {
      return mad_desc_isvalids
	(this->getPtr(), (n == 0) ? s.size() : n, s.c_str() );
    }
...
#if 0
    inline log_t isvalid(const ssz_t n, const str_t  s ) const
    { return mad_desc_isvalids  (this->getPtr(), n, s );  }
    inline log_t isvalid(const ssz_t n, const ord_t *m ) const
    { return mad_desc_isvalidm  (this->getPtr(), n, m );  }
    inline log_t isvalid(const ssz_t n, const idx_t *m ) const
    { return mad_desc_isvalidsm (this->getPtr(), n, m );  }
#endif
     ...
     */
    inline log_t idxsm(const std::vector<idx_t> m) const
    { return mad_desc_idxsm(this->getPtr(), m.size(), m.data()); }
#if 0
    inline idx_t idx(const ssz_t n, const str_t s ) const
    { return mad_desc_idxs      (this->getPtr(), n, s );  }
    inline idx_t idx(const ssz_t n, const ord_t *m ) const
    { return mad_desc_idxm      (this->getPtr(), n, m );  }
    inline idx_t idx(const ssz_t n, const idx_t *m ) const
    { return mad_desc_idxsm     (this->getPtr(), n, m );  }
#endif

    inline idx_t nxtbyvar(std::vector<ord_t> m ) const
    { return mad_desc_nxtbyvar(this->getPtr(), m.size(), m.data() );  }
    // inline idx_t nxtbyvar (ssz_t n, ord_t *m) const
    // { return mad_desc_nxtbyvar  (this->getPtr(), n, m );  }
    inline idx_t nxtbyord  (std::vector<ord_t> m ) const
    { return mad_desc_nxtbyord  (this->getPtr(), m.size(), m.data() );  }
    // inline idx_t nxtbyord  (ssz_t n, ord_t *m ) const
    // { return mad_desc_nxtbyord  (this->getPtr(), n, m );  }

    inline ord_t mono(const idx_t i, std::vector<ord_t>* m) const
    { return mad_desc_mono      (this->getPtr(), i, m->size(), m->data()); }
  ...
  inline std::ostream& operator<<(std::ostream& o, const desc& d)
  { d.show(o); return o; }
  inline std::ostream& operator<<(std::ostream& o, const desc_info& i)
  { i.show(o); return o; }
