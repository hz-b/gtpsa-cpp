    ...
    public:
        ...
        */
        ...
        inline ss_vect(const T &x, const T &px, const T &y, const T &py, const T &delta, const T &ct)
                : state_space{x, px, y, py, delta, ct} { this->checkSize(this->state_space); }
	...
	std::for_each(this->state_space.begin(), this->state_space.end(), [](gtpsa::tpsa& v){v = 0.0;});
        ...
        */
        ...
        inline void _copyInPlace(const ss_vect<T> &o) {
            std::transform(o.state_space.begin(), o.state_space.end(), this->state_space.begin(),
                           [](const T &elem) -> T { return elem.clone(); });
        }
        ...
        inline const std::vector<T> getConstVector(void) const {return this->state_space;}
...
//}
    ...
    inline ss_vect<double> ss_vect<double>::clone(void) const {
        ...
        nvec.reserve(size);
        std::transform(this->state_space.begin(), this->state_space.end(), nvec.begin(), [](const double &o) {
            double r = o;
            return r;
        });
    ...
    inline ss_vect<gtpsa::tpsa> operator+(const ss_vect<gtpsa::tpsa> &v1, const ss_vect<double> &v2) {
        auto r = v1.clone();
        r += v2;
        return r;
    }
